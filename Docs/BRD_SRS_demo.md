...
|---|---|---|---|---|
| FR-AUTH-01 | Đăng ký tài khoản Student | Student | Người dùng đăng ký tài khoản mới bằng email/mật khẩu để sử dụng nền tảng | **Given** người dùng chưa có tài khoản, **When** nhập email, mật khẩu hợp lệ và xác nhận, **Then** hệ thống tạo tài khoản, gửi email xác thực và chuyển hướng đến trang đăng nhập trong vòng 3 giây |
| FR-AUTH-02 | Đăng nhập hệ thống | Student, Teacher, Admin | Người dùng đăng nhập bằng email/mật khẩu đã đăng ký, hệ thống trả về token phân quyền theo vai trò | **Given** tài khoản đã tồn tại và đã xác thực email, **When** nhập đúng email và mật khẩu, **Then** hệ thống trả về JWT token hợp lệ và điều hướng vào giao diện tương ứng vai trò |
...
| **NFR-SEC-03** | Quản lý phiên & Xác thực (JWT) | Sử dụng JSON Web Token với cấu trúc phân tách rõ ràng giữa Access Token và Refresh Token | • Access Token: Thời hạn sống ngắn ($15 - 30\text{ phút}$)<br>• Refresh Token: Lưu HttpOnly, Secure cookie |
...
| **NFR-USE-03** | Thông báo lỗi thân thiện | Thông báo lỗi hiển thị bằng ngôn ngữ tự nhiên, chỉ dẫn rõ ràng cách khắc phục | Cung cấp mã lỗi cụ thể ở file CSV (ví dụ: *"Lỗi dòng 14: Thiếu cột 'Nghĩa của từ'"*) |
...
### 4.2. Bảng thuộc tính, kiểu dữ liệu, khóa chính/khóa ngoại

| Entity | Field | Kiểu dữ liệu | PK/FK | Ghi chú |
|---|---|---|---|---|
| User | user_id | BIGINT | PK | |
| User | full_name | VARCHAR(255) | | |
...
| Topic | topic_id | BIGINT | PK | |
| Topic | title | VARCHAR(255) | | |
| Topic | description | TEXT | | |
| Topic | level | VARCHAR(50) | | |
| Topic | teacher_id | BIGINT | FK → User.user_id | |
| Topic | created_at | DATETIME | | |
| Lesson | lesson_id | BIGINT | PK | |
| Lesson | topic_id | BIGINT | FK → Topic.topic_id | |
| Lesson | title | VARCHAR(255) | | |
| Lesson | order_index | INT | | Thứ tự hiển thị trong Topic |
| Lesson | completion_threshold | DECIMAL(5,2) | | Ngưỡng % tối thiểu để coi là hoàn thành |
| LessonMaterial | material_id | BIGINT | PK | |
| LessonMaterial | lesson_id | BIGINT | FK → Lesson.lesson_id | |
| LessonMaterial | type | ENUM('VIDEO','PDF','WORD') | | |
| LessonMaterial | file_url | VARCHAR(500) | | Signed URL/khóa truy cập |
| VocabularyItem | vocab_id | BIGINT | PK | |
| VocabularyItem | lesson_id | BIGINT | FK → Lesson.lesson_id | |
| VocabularyItem | word | VARCHAR(255) | | |
| VocabularyItem | meaning | VARCHAR(500) | | |
| VocabularyItem | pronunciation | VARCHAR(255) | | |
| VocabularyItem | example | TEXT | | |
| Minigame | minigame_id | BIGINT | PK | |
| Minigame | lesson_id | BIGINT | FK → Lesson.lesson_id | 1-1 hoặc 1-N tùy cấu hình |
| Minigame | status | ENUM('DRAFT','PUBLISHED') | | |
| MinigameQuestion | question_id | BIGINT | PK | |
| MinigameQuestion | minigame_id | BIGINT | FK → Minigame.minigame_id | |
| MinigameQuestion | vocab_id | BIGINT | FK → VocabularyItem.vocab_id | |
| MinigameQuestion | question_type | ENUM('MULTIPLE_CHOICE','MATCHING') | | |
| MinigameQuestion | correct_answer | VARCHAR(500) | | |
| MinigameAttempt | attempt_id | BIGINT | PK | |
| MinigameAttempt | minigame_id | BIGINT | FK → Minigame.minigame_id | |
| MinigameAttempt | student_id | BIGINT | FK → User.user_id | |
| MinigameAttempt | score | DECIMAL(5,2) | | |
| MinigameAttempt | attempted_at | DATETIME | | |
| LessonProgress | progress_id | BIGINT | PK | |
| LessonProgress | student_id | BIGINT | FK → User.user_id | |
| LessonProgress | lesson_id | BIGINT | FK → Lesson.lesson_id | |
| LessonProgress | status | ENUM('IN_PROGRESS','COMPLETED') | | |
| LessonProgress | updated_at | DATETIME | | |
| Comment | comment_id | BIGINT | PK | |
| Comment | author_id | BIGINT | FK → User.user_id | |
| Comment | topic_id | BIGINT | FK → Topic.topic_id (nullable) | Cấp Topic nếu không NULL |
| Comment | lesson_id | BIGINT | FK → Lesson.lesson_id (nullable) | Cấp Lesson nếu không NULL |
| Comment | parent_comment_id | BIGINT | FK → Comment.comment_id (nullable) | Self-reference cho reply |
| Comment | content | TEXT | | |
| Comment | created_at | DATETIME | | |

### 4.3. Mối quan hệ chính (Relationships)

| Quan hệ | Loại | Mô tả |
|---|---|---|
| User (Teacher) — Topic | 1–N | Một giáo viên quản lý nhiều chủ đề |
| Topic — Lesson | 1–N | Một chủ đề chứa nhiều bài học |
| Lesson — LessonMaterial | 1–N | Một bài học có thể chứa nhiều tài liệu (Video, PDF, Word) |
| Lesson — VocabularyItem | 1–N | Một bài học có nhiều từ vựng được parse từ CSV |
| Lesson — Minigame | 1–N | Một bài học có thể gắn một hoặc nhiều minigame |
| Minigame — MinigameQuestion | 1–N | Một minigame gồm nhiều câu hỏi |
| VocabularyItem — MinigameQuestion | 1–N | Một từ vựng có thể sinh ra một hoặc nhiều câu hỏi |
| User (Student) — MinigameAttempt | 1–N | Một học sinh có nhiều lượt làm minigame |
| Minigame — MinigameAttempt | 1–N | Một minigame có nhiều lượt làm từ nhiều học sinh |
| User (Student) — LessonProgress | 1–N | Một học sinh có tiến độ riêng cho từng bài học |
| Lesson — LessonProgress | 1–N | Một bài học được nhiều học sinh theo dõi tiến độ độc lập |
| User — Comment | 1–N | Một người dùng (Student/Teacher) có thể viết nhiều bình luận |
| Comment — Comment (self) | 1–N | Một bình luận gốc có thể có nhiều phản hồi (reply) |
